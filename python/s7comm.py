...
S7_ERROR_CLASS = {
    ...
    0xd209: "(At least) one of the given blocks not found on the module (0xd209)"
}
...
class S7GetBlockInfoDataReq(Packet):
    fields_desc = [
        ByteEnumField("ReturnCode", 0xff, S7_RETURN_CODE),
        ByteEnumField("TransportSize", 0x09, S7_TRANSPORT_SIZE_IN_DATA_ITEMS),
        FieldLenField("DataLength", 8, fmt="H", length_of="Blocks"),
        StrFixedLenEnumField("BlockType", "08", enum=S7_BLOCK_TYPE_IN_FILE_NAME, length=2),
...
class S7PIServiceParameterBlock(Packet):
    fields_desc = [
        ...
        StrFixedLenField("Unknown1", "\x00", length=1),
        FieldListField("FileNames", [], StrFixedLenField("FileName", "", length=8), count_from=lambda p: p.NumberOfBlocks)
...
def convert_transport_size_from_parm_to_data(parm_transport_size):
...
def pack_data_with_transport_size(req_item, data_list):
